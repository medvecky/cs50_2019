...
int main(int argc, char *argv[])
{
    ...
    if (n <= 0 || n > 100)
    {
        fprintf(stderr, "Factor %f is out of range valid value 1 .. 100.\n", n);
        return 1;
    }
    ...
    biNew.biWidth = round((float)biNew.biWidth * n);
    biNew.biHeight = round((float)biNew.biHeight * n);
    ...
    {
        // iterate over infile's scanlines
        for (int i = 0, biHeight = abs(biOrig.biHeight) - 1; i < biHeight; i++)
        {
            // attributes for increased line
            RGBTRIPLE *newLine = NULL;
            newLine = malloc(biNew.biWidth * sizeof(RGBTRIPLE));
            ...
            for (int j = 0; j < biOrig.biWidth; j++)
            {
                ...
                // assemble increased line
                newLine[(int)round((float)j * n)] = triple;
            }

            // write new line
            fseek(outptr, ((int)round((float)i * n)) * (biNew.biWidth * sizeof(RGBTRIPLE) + paddingNew) + 54, SEEK_SET);
            fwrite(newLine, biNew.biWidth * sizeof(RGBTRIPLE), 1, outptr);
            // add padding if needed
            for (int k = 0; k < paddingNew; k++)
            {
